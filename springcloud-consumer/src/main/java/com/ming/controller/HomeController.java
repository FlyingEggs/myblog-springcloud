...
import com.ming.dto.ArchiveDto;
import com.ming.dto.cond.ContentCond;
import com.ming.exception.BusinessException;
import com.ming.feignservice.FeignProviderService;
import com.ming.model.CommentDomain;
import com.ming.model.ContentDomain;
import com.ming.service.comment.CommentService;
import com.ming.service.content.ContentService;
import com.ming.service.meta.MetaService;
import com.ming.service.option.OptionService;
import com.ming.service.site.SiteService;
import com.ming.utils.*;
...
//        request.setAttribute("active","about");
    ...
    @ApiOperation("blog首页")
    @GetMapping(value = {"/myblog-provider/blog/","/myblog-provider/blog/index"})
...
//        return this.blogIndex(request, 1, limit);
    	return feignProviderService.blogIndex(request, limit);
    }

    @ApiOperation("blog首页-分页")
    @GetMapping(value = "/myblog-provider/blog/page/{p}")
    public String blogIndex(
            HttpServletRequest request,
            @PathVariable("p")
                    int p,
...
//        this.blogBaseData(request, contentCond);//获取公共分类标签等数据
    	feignProviderService.blogIndex(request, p, limit);
        return "site/blog";
    }

    @ApiOperation("文章内容页")
    @GetMapping(value = "/myblog-provider/blog/article/{cid}")
    public String post(
...
//        request.setAttribute("active","blog");
    	feignProviderService.post(cid, request);
...
//    }
    ...
    @GetMapping(value = "/myblog-provider/blog/archives/{date}")
...
//        this.blogBaseData(request, contentCond);//获取公共分类标签等数据
    	feignProviderService.archives(date, request);
    ...
    @GetMapping(value = "/myblog-provider/blog/archives/year/{year}")
...
//        request.setAttribute("archives_list",archives);
//        this.blogBaseData(request, contentCond);//获取公共分类标签等数据
    	feignProviderService.archivesAtYear(year, request);
        return "blog/archives";
    }
    ...
    @ApiOperation("归档页")
    @GetMapping(value = {"/myblog-provider/blog/archives", "/myblog-provider/blog/archives/index"})
...
//        request.setAttribute("archives_list",archives);
//        this.blogBaseData(request,contentCond);//获取公共分类标签等数据
    	feignProviderService.archives(request);
        return "blog/archives";
    }


    @ApiOperation("分类")
    @GetMapping(value = "/myblog-provider/blog/categories/{category}")
...
//        return  this.categories(category, 1, 10, request);
    	return feignProviderService.categories(category, request);
    }

    @ApiOperation("分类-分页")
    @GetMapping(value = "/myblog-provider/blog/categories/{category}/page/{page}")
    public String categories(
            @ApiParam(name = "category", value = "分类名", required = true)
            @PathVariable("category")
                    String category,
...
//        request.setAttribute("param_name", category);
    	feignProviderService.categories(category, page, limit, request);
        return "blog/categories";

    }

    @ApiOperation("标签页")
    @GetMapping(value = "/myblog-provider/blog/tag/{tag}")
    public String tags(
...
//        return this.tags(tag, 1, 10, request);
    	return feignProviderService.tags(tag, request);
    }

    @ApiOperation("标签页-分页")
    @GetMapping(value = "/myblog-provider/blog/tag/{tag}/page/{page}")
    public String tags(
            @ApiParam(name = "tag", value = "标签名", required = true)
            @PathVariable("tag")
                    String tag,
...
//        request.setAttribute("param_name", tag);
    	feignProviderService.tags(tag, page, limit, request);
        return "blog/categories";
    }

    @ApiOperation("搜索文章")
    @GetMapping("/myblog-provider/blog/search")
...
//        return this.search(param, 1, 10, request);
    	return feignProviderService.search(param, request);
    }

    @ApiOperation("搜索文章-分页")
    @GetMapping(value = "/myblog-provider/blog/search/{param}/page/{page}")
    public String search(
            @ApiParam(name = "param", value = "搜索的文字", required = true)
            @PathVariable("param")
                    String param,
...
//        request.setAttribute("param_name", param);
    	feignProviderService.search(param, page, limit, request);
        return "blog/index";
    }
     ...
     * 评论操作
     */
    @PostMapping(value = "/myblog-provider/blog/comment")
    @ResponseBody
...
//        }
    	return feignProviderService.comment(request, response, cid, coid, author, mail, url, text, _csrf_token);
    }
     ...
     * @param response
     */
    @RequestMapping("/myblog-provider/blog/logout")
    public void logout(HttpSession session, HttpServletResponse response) {
//        TaleUtils.logout(session, response);
    	feignProviderService.logout(session, response);
    }
    ...
    @ApiOperation("作品主页")
    @GetMapping(value = {"/myblog-provider/", "/myblog-provider/index"})
    public String index(HttpServletRequest request, @RequestParam(value = "limit", defaultValue = "12") int limit) {
//        return this.index(1, limit, request);
    	return feignProviderService.index(request, limit);
    }

    @ApiOperation("作品主页-分页")
    @GetMapping(value = "/myblog-provider/photo/page/{p}")
    public String index(
...
//        request.setAttribute("active", "work");
    	feignProviderService.index(request, page, limit);
        return "site/index";
    }


    @ApiOperation("作品内容")
    @GetMapping(value = "/myblog-provider/photo/article/{cid}")
...
//        request.setAttribute("active","work");
    	feignProviderService.article(cid, request);
